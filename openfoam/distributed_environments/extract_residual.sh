...
cat $LOG | grep 'Solving for epsilon' | cut -d' ' -f9 | tr -d ',' > ./$DIR/reseps_$PID.txt
cat $LOG | grep 'Solving for k' | cut -d' ' -f9 | tr -d ',' > ./$DIR/resk_$PID.txt
sed '/^$/d' ./$DIR/time_$PID.txt > ./$DIR/time1_$PID.txt
paste ./$DIR/time1_$PID.txt ./$DIR/resx_$PID.txt ./$DIR/resy_$PID.txt ./$DIR/resp_$PID.txt ./$DIR/reseps_$PID.txt ./$DIR/resk_$PID.txt | column -s $'\t' -t > ./$DIR/residual_$PID.txt
