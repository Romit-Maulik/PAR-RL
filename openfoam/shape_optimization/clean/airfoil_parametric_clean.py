...
target_airfoil_0012 = np.asarray(target_airfoil)
...
ax.plot(target_airfoil_0012[:,0], target_airfoil_0012[:,1], label='0012 Airfoil')
...
def shape_match(control_pts, xpts=xpts, ta=target_airfoil_0012):
...
np.savetxt('naca0012_cp_optimized.csv',res.x,delimiter=",")

#%%
# lets plot the solution
#res.x[0] = 0.25
matched_airfoil = np.array(bezier_airfoil(xpts, munge_ctlpts(res.x, q, q)))
x0, y0 = zip(*target_airfoil_0012)
...
parameterized_airfoil_0012 = np.array(bezier_airfoil(x_cos, munge_ctlpts(res.x, q, q)))

np.savetxt('parameterized_airfoil_0012.csv',parameterized_airfoil_0012,delimiter=",")

#%%
# final sanity check
x0, y0 = zip(*matched_airfoil)
x1, y1 = zip(*parameterized_airfoil_0012)
...
parameterized_airfoil_2412 = np.array(bezier_airfoil(x_cos, munge_ctlpts(res_x, q, q)))

np.savetxt('parameterized_airfoil_2412.csv',parameterized_airfoil_2412,delimiter=",")
x1, y1 = zip(*parameterized_airfoil_2412)
...
#%%
# writes airfoil cordinates onto input sheet
# matched_airfoil_cos = np.loadtxt('airfoil_cos.npy')
pid = 1
origcase = 'baseCase'
...
with open(f'log.convert', 'a') as fp:
    subprocess.run(
                    f'cp blockMeshDictGenerator.xlsx ./{casename}/blockMeshDictGenerator.xlsx',
                    shell=True,
                    stdout=fp,
...
wb = load_workbook(f'./{casename}/blockMeshDictGenerator.xlsx')
ws = wb['Input'] 
shift = 9

#for row in ws.iter_rows(min_row=shift+1, min_col=1, max_col=2):
#    row[0] = matched_airfoil_cos[0,0]

for index, data in enumerate(parameterized_airfoil_0012):
    ws.cell(row=index+shift, column=1).value = data[0]
    ws.cell(row=index+shift, column=2).value = data[1]

    
wb.save(f'./{casename}/blockMeshDictGenerator.xls')

with open(f'log.convert', 'a') as fp:
    subprocess.run(
                    f'libreoffice --headless --convert-to xlsx ./{casename}/*.xls --outdir ./{casename}',
                    shell=True,
                    stdout=fp,
                    stderr=subprocess.STDOUT
                )        
...
wb = openpyxl.load_workbook(f'./{casename}/blockMeshDictGenerator.xlsx',data_only=True)
sh = wb['blockMesh'] 

with open(f'./{casename}/blockMeshDictGenerator.csv', 'w') as f:  
    c = csv.writer(f)
    for r in sh.rows:
...
#%% 
# find out a way to read values and not formulas in Pandas
import pandas as pd
pd.set_option("display.max_colwidth", 100)
#df = pd.read_excel(r'blockMeshDictGenerator_v3.xlsx',sheet_name = 'blockMesh' , header=None)
df = pd.read_csv(f'./{casename}/blockMeshDictGenerator.csv',header=None)
df = df.fillna('')
...
base_filename = f'./{casename}/system/blockMeshDict'
import csv    
#df1.to_csv(base_filename, header=None, index=None, quoting=csv.QUOTE_NONE, sep=' ', escapechar = ' ')
...
# to run on theta       
proc = subprocess.Popen([f'$FOAM_APPBIN/{mesh} {solveroptions} {casename} >> {casename}/{meshLogFile}'],
                shell=True,
...
# to run on theta       
proc = subprocess.Popen([f'$FOAM_APPBIN/{solver} {solveroptions} {casename} >> {casename}/{solverLogFile}'],
                shell=True,
                stdout=subprocess.PIPE, 
                stderr=subprocess.PIPE)
...
#%%
casename = 'baseCase_1'
force_coeffs = np.loadtxt(f'./{casename}/postProcessing/forceCoeffsIncompressible/0/forceCoeffs.dat')
