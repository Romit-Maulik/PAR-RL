...
#Source code :https://curiosityfluids.com/2019/04/22/automatic-airfoil-cmesh-generation-for-openfoam-rev-1/
...
parser = argparse.ArgumentParser(description = 'path of the airfoil_data.dat file')
parser.add_argument('-p', '--path', type = str, help = 'Path of the data file')
args = parser.parse_args()

ChordLength=1
airfoilfile=os.path.join(args.path, 'airfoil_data.dat')
...
Coords=np.loadtxt(airfoilfile)	#Import airfoil coordinates
...
f = open(os.path.join(args.path, 'system/blockMeshDict') ,'w')
f.write('/*--------------------------------*- C++ -*----------------------------------*\ \n')
f.write('  =========                 | \n')
